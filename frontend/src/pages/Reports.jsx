  ...
  return (
    <div className="p-8 w-full max-w-7xl mx-auto pb-24">
      <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 print:hidden">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2 tracking-tight">Reports & Analytics</h1>
          <p className="text-gray-400">Deep insights into training performance and compliance.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => window.print()} className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg font-medium transition-colors border border-gray-700 shadow-lg">
      ...
      <div className="bg-gradient-to-r from-gray-900 to-[#181818] rounded-2xl p-6 border border-gray-800 mb-8 shadow-2xl relative overflow-hidden print:hidden">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Ask HK Training AI</h2>
        ...
        <form onSubmit={handleAskAI} className="relative">
          <input 
            ...
            className="w-full bg-[#121212] border border-gray-700 rounded-xl pl-4 pr-16 py-4 text-white focus:border-brand-gold outline-none shadow-inner"
          />
          <button 
            type="submit" 
            disabled={isAiThinking}
            className="absolute right-2 top-1/2 -translate-y-1/2 bg-brand-gold hover:bg-brand-goldHover text-black p-2.5 rounded-lg font-bold transition-colors shadow-md disabled:opacity-50"
          >
      ...
      {selectedReportTab !== 'ai-report' && (
        <div className="bg-brand-card rounded-xl p-5 border border-gray-800 mb-8 flex flex-wrap items-end gap-4 shadow-xl print:hidden">
          <div>
            ...
            <input type="date" value={dateRange.start} onChange={(e) => setDateRange({...dateRange, start: e.target.value})} className="bg-[#181818] border border-gray-700 rounded-lg p-2 text-white focus:border-brand-gold outline-none" style={{colorScheme:'dark'}}/>
          </div>
          <div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setSelectedReportTab('detailed-summary')} className="bg-gray-800 text-white hover:bg-gray-700 border border-gray-700 px-6 py-2 rounded-lg font-medium transition-colors flex items-center gap-2">
              <Eye className="w-4 h-4" />
              View
            </button>
            <button onClick={() => window.print()} className="bg-gray-800 text-white hover:bg-gray-700 border border-gray-700 px-6 py-2 rounded-lg font-medium transition-colors flex items-center gap-2">
              <Printer className="w-4 h-4" />
              Print
            </button>
          </div>
        ...
        {['overview', 'attendance', 'ojt-performance', 'detailed-summary', 'birthday-calendar', 'ai-report'].map((tab) => {
          ...
          return (
            <button
              ...
              className={`px-6 py-3 font-medium text-sm transition-colors whitespace-nowrap flex items-center gap-2 ${selectedReportTab === tab ? 'text-brand-gold border-b-2 border-brand-gold bg-brand-gold/5' : 'text-gray-400 hover:text-white'}`}
            >
          ...
          {isAiThinking ? (
            ...
            <div className="bg-brand-card print:bg-white rounded-2xl border border-gray-800 print:border-none shadow-lg print:shadow-none overflow-hidden print:block">
              ...
              <div className="p-6 border-b border-gray-800 print:border-gray-400 bg-[#181818] print:bg-white print:text-center print:mb-6 flex justify-between items-center">
                <div>
                  <h2 className="text-2xl print:text-3xl font-bold text-white print:text-black flex items-center gap-2 print:justify-center print:mb-2">
                    <Sparkles className="w-6 h-6 text-brand-gold print:hidden" /> {aiResult.title}
                  </h2>
                        ...
                        <tr key={i} className="hover:bg-gray-800/30 print:hover:bg-transparent transition-colors">
                          {Object.values(row).map((val, j) => (
                            <td key={j} className={`p-4 border border-gray-800 print:border-gray-300 text-sm font-medium ${
                              ...
                              val === 'PASS' ? 'text-green-400 print:text-black' : 'text-white print:text-black'
                            }`}>
                              {val}
                            </td>
                          ))}
        ...
        <div className="space-y-8 print:hidden">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-brand-card rounded-2xl p-6 border border-gray-800 shadow-lg">
              <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2"><BarChart className="w-5 h-5 text-brand-gold"/> Monthly Training Hours</h2>
              <div className="h-64">
            ...
            <div className="bg-brand-card rounded-2xl p-6 border border-gray-800 shadow-lg">
              <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2"><PieChart className="w-5 h-5 text-brand-gold"/> Dept. Training Hours (Housekeeping)</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-brand-card rounded-2xl border border-gray-800 shadow-lg overflow-hidden">
              <div className="p-6 border-b border-gray-800 bg-[#181818]">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><UserX className="w-5 h-5 text-red-400"/> Topic-wise Absenteeism</h2>
              </div>
                  ...
                  <tbody>
                    {absentData.map((row, i) => (
                      <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        <td className="p-4 text-white font-medium">{row.topic}</td>
            ...
            <div className="bg-brand-card rounded-2xl border border-gray-800 shadow-lg overflow-hidden">
              <div className="p-6 border-b border-gray-800 bg-[#181818]">
                <h2 className="text-lg font-bold text-white flex items-center gap-2"><AlertCircle className="w-5 h-5 text-orange-400"/> Employees Missing Topics</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                  ...
                  <tbody>
                    {missingTopicsData.map((row, i) => (
                      <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        <td className="p-4 text-white font-medium">{row.employee}</td>
        ...
        <div className="space-y-8 print:hidden">
          <div className="bg-red-500/5 border border-red-500/20 rounded-2xl shadow-lg overflow-hidden relative">
            ...
            <div className="p-6 border-b border-red-500/10 bg-red-500/5">
              <h2 className="text-lg font-bold text-white flex items-center gap-2"><AlertCircle className="w-5 h-5 text-red-500"/> Low Performance OJT Assessments</h2>
                  ...
                  {lowPerformanceOJT.map((row, i) => (
                    <tr key={i} className="border-b border-red-500/10 hover:bg-red-500/10 transition-colors">
                      ...
                      <td className="p-4 flex gap-1">
                        {[1, 2, 3, 4, 5].map(star => (
                          <svg key={star} className={`w-4 h-4 ${star <= row.rating ? 'text-brand-gold fill-current' : 'text-gray-700 stroke-current fill-none'}`} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
                        ))}
                      </td>
          ...
          <div className="bg-brand-card rounded-2xl border border-gray-800 shadow-lg overflow-hidden">
            <div className="p-6 border-b border-gray-800 bg-[#181818] flex justify-between items-center">
              <h2 className="text-lg font-bold text-white flex items-center gap-2"><FileText className="w-5 h-5 text-brand-gold"/> Employee OJT Master Record</h2>
              <div className="relative">
                <Search className="w-4 h-4 text-gray-500 absolute left-3 top-1/2 -translate-y-1/2" />
                <input type="text" placeholder="Search employee..." className="bg-gray-900 border border-gray-700 rounded-lg pl-9 pr-4 py-2 text-sm text-white focus:border-brand-gold outline-none" />
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {ojtDetails.map((row, i) => (
                    <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="p-4 text-brand-gold font-bold">{row.avgRating} <span className="text-xs text-gray-500">/ 5</span></td>
                      <td className="p-4 text-gray-400 text-sm">{row.lastTopic}</td>
                      <td className="p-4">
                        <button className="text-brand-gold hover:text-white text-sm font-medium transition-colors">View Full History</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="bg-brand-card print:bg-white rounded-2xl border border-gray-800 print:border-none shadow-lg print:shadow-none overflow-hidden print:block p-8">
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-gray-800 print:border-gray-400 pb-6 gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-3xl font-bold text-white print:text-black">Monthly Birthday Calendar</h2>
              ...
              <div className="print:hidden">
                <select 
                  ...
                  className="bg-[#181818] border border-gray-700 rounded-lg p-3 text-white focus:border-brand-gold outline-none font-bold text-lg"
                >
                  ...
                  {(() => {
                    ...
                    return monthBirthdays.map((emp) => (
                      <tr key={emp.id} className="hover:bg-gray-800/50 print:hover:bg-transparent transition-colors">
                        ...
                        <td className="p-4 border-b border-gray-800 print:border-gray-300 text-white print:text-black font-medium">{emp.full_name}</td>
      ...
      {(selectedReportTab === 'detailed-summary' || (typeof window !== 'undefined' && window.matchMedia('print').matches && selectedReportTab !== 'ai-report')) && (
        <div className={`${selectedReportTab === 'detailed-summary' ? 'block' : 'hidden'} print:block w-full bg-brand-card print:bg-white border border-gray-800 print:border-none rounded-2xl p-8 print:p-0`}>
          <div className="text-center mb-8 border-b border-gray-800 print:border-b-2 print:border-black pb-6">
            <h1 className="text-3xl print:text-4xl font-bold uppercase tracking-widest text-white print:text-black mb-2">Housekeeping Department Trainings</h1>
            <p className="text-brand-gold print:text-gray-600 font-medium">Monthly Training & Compliance Report</p>
            <p className="text-sm text-gray-500 mt-1">Generated: {new Date().toLocaleDateString()}</p>
          </div>

          <div className="mb-8">
            <h2 className="text-xl font-bold border-b border-gray-700 print:border-gray-400 mb-4 pb-2 text-white print:text-black">Department wise SOP training</h2>
                ...
                <tbody>
                  {printDataSOP.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-800/50 print:hover:bg-transparent">
                      <td className="p-3 border-b border-gray-800 print:border-gray-300 text-sm text-white print:text-black">{row.topic}</td>
                      <td className="p-3 border-b border-gray-800 print:border-gray-300 text-sm text-white print:text-black">{row.sessions}</td>
                      <td className="p-3 border-b border-gray-800 print:border-gray-300 text-sm text-white print:text-black">{row.attendees}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="mb-8">
            <h2 className="text-xl font-bold border-b border-gray-700 print:border-gray-400 mb-4 pb-2 text-white print:text-black">OJT</h2>
            <div className="overflow-hidden rounded-lg border border-gray-800 print:border-gray-300">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-900 print:bg-gray-100">
                ...
                <tbody>
                  {printDataOJT.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-800/50 print:hover:bg-transparent">
                      <td className="p-3 border-b border-gray-800 print:border-gray-300 text-sm text-white print:text-black">{row.topic}</td>
                      <td className="p-3 border-b border-gray-800 print:border-gray-300 text-sm text-white print:text-black">{row.assessed}</td>
                      <td className="p-3 border-b border-gray-800 print:border-gray-300 text-sm text-brand-gold print:text-black">{row.passed}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="mb-8">
            <h2 className="text-xl font-bold border-b border-gray-700 print:border-gray-400 mb-4 pb-2 text-white print:text-black">Hotel HR Training</h2>
            <div className="overflow-hidden rounded-lg border border-gray-800 print:border-gray-300">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-900 print:bg-gray-100">
          ...
          <div className="mb-8">
            <h2 className="text-xl font-bold border-b border-gray-700 print:border-gray-400 mb-4 pb-2 text-white print:text-black">Training Hours Summary</h2>
            <div className="overflow-hidden rounded-lg border border-gray-800 print:border-gray-300">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-900 print:bg-gray-100">
                ...
                <tbody>
                  {printDataHours.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-800/50 print:hover:bg-transparent">
                      <td className="p-3 border-b border-gray-800 print:border-gray-300 text-sm font-bold text-brand-gold print:text-black">{row.category}</td>
                      <td className="p-3 border-b border-gray-800 print:border-gray-300 text-sm text-white print:text-black">{row.hours}h</td>
                    </tr>
                  ))}
                </tbody>
              </table>
