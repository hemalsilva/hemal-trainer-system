    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-900 p-6 rounded-2xl border border-gray-800 shadow-xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <select value={month} onChange={(e) => setMonth(parseInt(e.target.value))} className="bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2">
            {Array.from({ length: 12 }, (_, i) => (
              <option key={i+1} value={i+1}>{new Date(2000, i).toLocaleString('default', { month: 'long' })}</option>
            ))}
          </select>
          <select value={year} onChange={(e) => setYear(parseInt(e.target.value))} className="bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2">
              ...
              {loading ? (
                ...
                records.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="p-4 font-medium text-white">{record.topic}</td>
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => handleDownload(record.id)} className="p-2 text-gray-400 hover:text-brand-gold hover:bg-gray-800 rounded-lg transition-colors" title="Download PDF">
      ...
      <div className="bg-gray-900 rounded-2xl border border-gray-800 p-8 shadow-xl text-center">
        <Upload className="w-12 h-12 text-brand-gold mx-auto mb-4" />
        <h2 className="text-2xl font-bold text-white mb-2">Bulk Upload Employee Photos</h2>
        ...
        <label className="bg-brand-gold hover:bg-brand-goldHover text-black font-bold py-3 px-8 rounded-xl cursor-pointer inline-flex items-center gap-2 transition-all shadow-lg hover:shadow-brand-gold/20 hover:-translate-y-0.5">
