  ...
  return (
    <div className="p-8 w-full max-w-7xl mx-auto pb-24">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2 tracking-tight flex items-center gap-3">
            <ClipboardList className="w-8 h-8 text-brand-gold" /> Questionnaire Results
          </h1>
          <p className="text-gray-400">Track post-training quiz scores � by date, department, and employee.</p>
        </div>
        <button onClick={() => setShowModal(true)} className="bg-brand-gold hover:bg-brand-goldHover text-black px-5 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-brand-gold/20 transition-all hover:-translate-y-0.5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-brand-card border border-gray-800 rounded-2xl p-5">
          <div className="flex items-center gap-3 mb-3"><BarChart2 className="w-5 h-5 text-brand-gold" /><span className="text-gray-400 text-sm">Average Score</span></div>
      ...
      {deptStats.length > 0 && (
        <div className="bg-brand-card border border-gray-800 rounded-2xl p-6 mb-8">
          <h2 className="text-white font-bold mb-4 flex items-center gap-2"><BarChart2 className="w-5 h-5 text-brand-gold" /> Average Score by Department</h2>
            ...
            {deptStats.map(d => {
              ...
              return (
                <div key={d.dept} className={`rounded-xl p-4 border ${c.border} ${c.bg}`}>
                  <p className="text-white font-semibold text-sm mb-1 truncate">{d.dept}</p>
      ...
      <div className="bg-brand-card border border-gray-800 rounded-2xl p-4 mb-6 flex flex-wrap gap-3 items-center">
        <Filter className="w-5 h-5 text-brand-gold flex-shrink-0" />
        <div className="relative flex-1 min-w-[160px]">
          <Search className="w-4 h-4 text-gray-500 absolute left-3 top-1/2 -translate-y-1/2" />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name / emp no..." className="w-full bg-gray-900 border border-gray-700 rounded-lg pl-9 pr-3 py-2 text-white text-sm focus:border-brand-gold outline-none" />
        </div>
        <select value={filterDept} onChange={e => setFilterDept(e.target.value)} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:border-brand-gold outline-none">
          <option value="">All Departments</option>
          {DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)}
        </select>
        <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:border-brand-gold outline-none" style={{colorScheme:'dark'}} />
        <select value={filterTopic} onChange={e => setFilterTopic(e.target.value)} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:border-brand-gold outline-none">
          <option value="">All Topics</option>
          {topics.map(t => <option key={t} value={t}>{t}</option>)}
        </select>
        <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:border-brand-gold outline-none">
      ...
      {filtered.length === 0 ? (
        <div className="bg-brand-card border border-dashed border-gray-700 rounded-2xl p-16 text-center">
          ...
          <button onClick={() => setShowModal(true)} className="bg-brand-gold text-black px-6 py-3 rounded-xl font-bold hover:bg-brand-goldHover transition-colors">
                  ...
                  return (
                    <tr key={r.id} className={`hover:bg-gray-800/30 transition-colors ${c.bg}`}>
                      <td className="px-4 py-3">
                        <p className="text-white font-semibold">{r.emp_name}</p>
          ...
          <div className="flex flex-wrap gap-2">
            {lowScorers.map(r => (
              <div key={r.id} className="bg-red-900/30 border border-red-700/40 rounded-lg px-3 py-2 text-sm">
                <span className="text-white font-semibold">{r.emp_name}</span>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-brand-card border border-gray-800 rounded-2xl w-full max-w-lg p-8 relative shadow-2xl">
            ...
            <h2 className="text-2xl font-bold text-white mb-1 flex items-center gap-2"><ClipboardList className="text-brand-gold w-6 h-6" /> Add Quiz Result</h2>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required value={form.emp_no} onChange={e => setForm({...form, emp_no: e.target.value})} placeholder="EMP-001" className="w-full bg-[#181818] border border-gray-700 rounded-xl p-3 text-white focus:border-brand-gold outline-none" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1.5">Employee Name *</label>
                  <input required value={form.emp_name} onChange={e => setForm({...form, emp_name: e.target.value})} placeholder="Full Name" className="w-full bg-[#181818] border border-gray-700 rounded-xl p-3 text-white focus:border-brand-gold outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-1.5">Department *</label>
                <select required value={form.department} onChange={e => setForm({...form, department: e.target.value})} className="w-full bg-[#181818] border border-gray-700 rounded-xl p-3 text-white focus:border-brand-gold outline-none">
              ...
              <div>
                ...
                <input required value={form.topic} onChange={e => setForm({...form, topic: e.target.value})} placeholder="e.g. Fire Safety Protocol" className="w-full bg-[#181818] border border-gray-700 rounded-xl p-3 text-white focus:border-brand-gold outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1.5">Training Date *</label>
                  <input required type="date" value={form.training_date} onChange={e => setForm({...form, training_date: e.target.value})} className="w-full bg-[#181818] border border-gray-700 rounded-xl p-3 text-white focus:border-brand-gold outline-none" style={{colorScheme:'dark'}} />
                </div>
                <div>
                  ...
                  <input required type="number" min="0" max="100" value={form.score} onChange={e => setForm({...form, score: e.target.value})} placeholder="e.g. 85"
                    className={`w-full bg-[#181818] border rounded-xl p-3 text-white focus:ring-1 outline-none transition-all font-bold text-xl text-center ${form.score !== '' ? scoreColor(parseInt(form.score)||0).border : 'border-gray-700 focus:border-brand-gold'}`} />
                </div>
              </div>
              {form.score !== '' && (
              ...
              <div className="pt-2 flex gap-3">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 border border-gray-700 text-gray-400 hover:text-white py-3 rounded-xl font-semibold">Cancel</button>
                <button type="submit" className="flex-1 bg-brand-gold text-black py-3 rounded-xl font-bold hover:bg-brand-goldHover">Save Result</button>
              </div>
            </form>
          </div>
        </div>
