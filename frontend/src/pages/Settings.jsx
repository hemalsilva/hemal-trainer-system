        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-1 space-y-6">
            <div className="bg-brand-card rounded-2xl p-6 border border-gray-800 shadow-lg">
              <h2 className="text-xl font-bold text-white mb-4">1. Upload Template</h2>
              <p className="text-sm text-gray-400 mb-4">Upload a blank certificate design (PNG/JPG). Make sure it's high quality.</p>
              
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-700 border-dashed rounded-xl cursor-pointer hover:bg-gray-800/50 hover:border-brand-gold transition-colors">
            ...
            <div className="bg-brand-card rounded-2xl p-6 border border-gray-800 shadow-lg">
              <h2 className="text-xl font-bold text-white mb-4">2. Calibrate Fields</h2>
              <p className="text-sm text-gray-400 mb-4">Select a tag below, then click on the blank certificate to place it.</p>
              
              <div className="space-y-3">
                {[
                  { id: 'name', label: '[Employee Name]', color: 'bg-blue-500/20 text-blue-400 border-blue-500/50' },
                  { id: 'topic', label: '[Training Topic]', color: 'bg-purple-500/20 text-purple-400 border-purple-500/50' },
                  { id: 'date', label: '[Date]', color: 'bg-green-500/20 text-green-400 border-green-500/50' }
                ].map((tag) => (
                  <button
                    key={tag.id}
                    ...
                    className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${
                      activeTag === tag.id 
                        ? 'border-brand-gold bg-brand-gold/10 shadow-[0_0_10px_rgba(212,175,55,0.2)]' 
                        : 'border-gray-700 bg-[#181818] hover:bg-gray-800'
                    }`}
                  >
                    <span className={`font-mono text-sm px-2 py-1 rounded border ${tag.color}`}>{tag.label}</span>
                    {activeTag === tag.id ? <MousePointer2 className="w-4 h-4 text-brand-gold animate-pulse" /> : null}
                  </button>
                ))}
              </div>
            </div>
            ...
            <button 
              onClick={saveConfig}
              className="w-full bg-brand-gold hover:bg-brand-goldHover text-black py-4 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(212,175,55,0.2)] flex items-center justify-center gap-2"
            >
          ...
          {/* Preview Area */}
          <div className="xl:col-span-2 bg-brand-card rounded-2xl border border-gray-800 shadow-lg p-6 flex flex-col items-center justify-center min-h-[600px] relative overflow-hidden">
            {!template ? (
              <div className="text-center text-gray-500 flex flex-col items-center">
                <ImageIcon className="w-16 h-16 mb-4 opacity-50" />
                <p>No template uploaded yet.</p>
                <p className="text-sm mt-2">Upload an image on the left to begin calibration.</p>
              </div>
            ) : (
              <div className="relative w-full max-w-4xl shadow-2xl border border-gray-700">
                {/* Instruction overlay if tag is selected */}
                {activeTag && (
                  <div className="absolute inset-0 bg-brand-gold/5 z-10 cursor-crosshair flex items-center justify-center" onClick={handleImageClick}>
                    <div className="bg-black/80 text-white px-4 py-2 rounded-full font-medium shadow-xl pointer-events-none backdrop-blur-sm border border-brand-gold/30">
                      Click anywhere on the certificate to place {activeTag.toUpperCase()}
                    </div>
                  </div>
                )}
                ...
                <img 
                  ...
                  onClick={activeTag ? handleImageClick : undefined}
                />
                
                {/* Visual Markers */}
                <div 
                  className="absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center pointer-events-none"
                  style={{ left: `${coords.name.x}%`, top: `${coords.name.y}%`, zIndex: 20 }}
                >
                  <div className="w-3 h-3 bg-blue-500 rounded-full border-2 border-white shadow-md"></div>
                  <span className="mt-1 bg-blue-500 text-white text-[10px] px-2 py-0.5 rounded font-mono font-bold shadow-md whitespace-nowrap opacity-80">[Name]</span>
                </div>
                
                <div 
                  className="absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center pointer-events-none"
                  style={{ left: `${coords.topic.x}%`, top: `${coords.topic.y}%`, zIndex: 20 }}
                >
                  <div className="w-3 h-3 bg-purple-500 rounded-full border-2 border-white shadow-md"></div>
                  <span className="mt-1 bg-purple-500 text-white text-[10px] px-2 py-0.5 rounded font-mono font-bold shadow-md whitespace-nowrap opacity-80">[Topic]</span>
                </div>
                
                <div 
                  className="absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center pointer-events-none"
                  style={{ left: `${coords.date.x}%`, top: `${coords.date.y}%`, zIndex: 20 }}
                >
                  <div className="w-3 h-3 bg-green-500 rounded-full border-2 border-white shadow-md"></div>
                  <span className="mt-1 bg-green-500 text-white text-[10px] px-2 py-0.5 rounded font-mono font-bold shadow-md whitespace-nowrap opacity-80">[Date]</span>
                </div>
              </div>
            )}
          </div>
