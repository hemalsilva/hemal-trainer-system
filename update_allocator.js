...
// 1. Rewrite handleGenerateAllocations
const oldGen = \const handleGenerateAllocations = () => {\;
const newGen = \const handleGenerateAllocations = async () => {\;
content = content.replace(oldGen, newGen);

const oldGenLogic = \const uploadedDepts = Object.keys(monthlyRosters);
...
const newGenLogic = \const uploadedDepts = Object.keys(monthlyRosters);
    if (uploadedDepts.length === 0) {
      alert('Please upload at least one department roster first.');
      return;
    }
...
\;
// Insert before handleSaveSync
content = content.replace('const handleSaveSync = async () => {', exportFuncs + '\\n  const handleSaveSync = async () => {');

// 3. Add Export button in Sync Modal
const oldSyncButtons = \<button onClick={() => setShowSyncModal(false)} className="px-5 py-2.5 rounded-lg border border-gray-700 text-gray-300 hover:bg-gray-800 transition-colors font-medium">Cancel</button>
...
const newSyncButtons = \<button onClick={() => setShowSyncModal(false)} className="px-5 py-2.5 rounded-lg border border-gray-700 text-gray-300 hover:bg-gray-800 transition-colors font-medium">Cancel</button>
...
content = content.replace(oldSyncButtons, newSyncButtons);

// 4. Update Print CSS & Buttons
const oldPrintButton = \<button onClick={() => window.print()} className="bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-300 px-4 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-colors shadow-lg">
            <Printer className="w-5 h-5" /> Print
          </button>\;
const newPrintButton = \<button onClick={() => { document.body.classList.add('print-calendar-only'); window.print(); document.body.classList.remove('print-calendar-only'); }} className="bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-300 px-4 py-2.5 rounded-lg font-medium flex items-center gap-2 transition-colors shadow-lg">
            <Printer className="w-5 h-5" /> Print Calendar
          </button>\;
content = content.replace(oldPrintButton, newPrintButton);

const oldDetailedHeader = \<div className="flex items-center justify-between p-6 border-b border-gray-800 bg-gray-800/50">
          <div>
...
const newDetailedHeader = \<div className="flex items-center justify-between p-6 border-b border-gray-800 bg-gray-800/50 print:hidden">
          <div>
            <h2 className="text-xl font-bold text-blue-200 flex items-center gap-3"><BookOpen className="w-6 h-6 text-brand-primary" /> Detailed Training Schedule</h2>
            <p className="text-sm text-gray-400 mt-1">Detailed list view for the selected month and department.</p>
          </div>
...
content = content.replace(oldDetailedHeader, newDetailedHeader);

// 5. Add custom print CSS inside the component using a style tag
const styleTag = \
      <style>{\\\
        @media print {
...
content = content.replace('<header className="mb-8 flex', styleTag + '<header className="mb-8 flex');

// Add classnames to sections
content = content.replace('      {/* BIG CALENDAR GRID */}', '      {/* BIG CALENDAR GRID */}\\n      <div className="calendar-section">');
// Close the div after the grid
...
console.log('Allocator and export logic updated in Schedule.jsx');
