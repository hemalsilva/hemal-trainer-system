...
// 1. Add state for completionFilter
...
// 2. Update filteredOverview logic
...
// 3. Add the dropdown
const oldHeading = `<div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-blue-200">Employee Audit Scores</h3>
              <select`;

const newHeading = `<div className="flex flex-wrap items-center justify-between gap-4 mb-4">
              <h3 className="text-lg font-bold text-blue-200">Employee Audit Scores</h3>
...
reports = reports.replace(oldHeading, newHeading);

// ensure closing div is matched correctly?
...
// with my replacement it will be:
...
//     </select>
//   </div>
// </div> // wait I need to close the inner div!

const innerDivCloseOld = `</select>
            </div>`;
const innerDivCloseNew = `</select>
              </div>
            </div>`;

// Actually let's just do a cleaner replace by replacing the whole block.
// Let me read the file and replace properly.
